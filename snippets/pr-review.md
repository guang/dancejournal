...
> **Reviewed through:** `dc0fb9a` — the last commit this review covers
...
- **Scroll the nav on mobile (≤768px)** — Load each page at a narrow width. **Watch:** "What's New" is absent at the top, slides in from the wordmark's vacated slot once `.dz-nav.scrolled` engages (~modest scroll distance), no layout jump or overlap with the CTA. This is the untested crossfade behavior called out in the PR description — verify on a real narrow viewport, not just the diff.
